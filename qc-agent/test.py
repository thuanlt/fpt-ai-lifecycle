...
def _get_sort_key(item: Dict[str, Any]) -> float:
    try:
...
class TestProcessor:
    ...
    def process_test_cycle(self, test_cycle_id: int):
        """
        ...
        all_test_run_items.sort(key=_get_sort_key)
        ...
        for item in all_test_run_items:
            ...
            if not final_step_list:
                ...
                # for i, step in enumerate(final_step_list):
                #     print(f"  Step {i+1}: {step.get('description')}")
                #     print(f"    Expected: {step.get('expectedResult')}")
            print(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    # Run the process
    processor.process_test_cycle(target_test_cycle_id)