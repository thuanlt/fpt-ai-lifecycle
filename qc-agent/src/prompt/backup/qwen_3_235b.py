...
class Qwen3_235bPrompt(Prompt):
    ...
    def _PROMPT_TEMPLATE(self, *args, **kwargs):
        """Returns the orchestration prompt template."""
        return """# SYSTEM PROMPT
You MUST THINK CAREFULLY to FOLLOW EXACTLY INSTRUCTION
{% if role %}
## ROLE
{{ role }}
{% endif %}
...
{% if constraints %}
## CONSTRAINTS
{% if constraints is iterable and not constraints is string %}
...
{% if related_information and "No relevant information found." not in related_information %}
...
# {# Vòng lặp qua tất cả các item trong context được truyền vào #}
...
# # WORKING MEMORY AND CONTEXT
# {# --- PHẦN BỘ NHỚ CỐ ĐỊNH --- #}
# {# Các trường này được xử lý riêng vì chúng có tiêu đề dài và cố định #}
# {% if related_information and "No relevant information found." not in related_information %}
# ## RELATED INFORMATION FROM LONG-TERM MEMORY - SORTED BY DECREASING RELEVANCE
# {{ related_information }}
# {% endif %}
# {% if recent_conversation and "No recent conversation history." not in recent_conversation %}
# ## RECENT CONVERSATION HISTORY - SORTED BY TIME, OLDEST FIRST, MOST RECENT LAST
# {{ recent_conversation }}
# {% endif %}
# {% if shared_log and shared_log != "N/A" %}
...
#     def QC_AGENT_PROFILE(self, *args, **kwargs):
#         """Returns the QC Agent's profile."""
#         return {
#             "name": "QCAgent",
#             "role": "AI Quality Control (QC) Executor",
#             "target": "Execute a single test step accurately using the available tools.",
#             "ability": "Calls functions (tools) via the MCP client with the provided parameters.",
#             "personality": "Agile, precise, and task-focused.",
#             "constraints": "Only execute the requested tool. Do not interpret or modify the parameters.",
#             "output_format": "The execution result will be returned from the MCP client."
#         }
