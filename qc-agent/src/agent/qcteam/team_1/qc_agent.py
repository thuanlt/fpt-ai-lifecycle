...
class QCAgent(AbstractAgent):
    ...
    def agent_as_a_function_description(self) -> dict:
        return {
            ...
            "function": {
                ...
                "description": "Execute a quality control task using available tools.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "task_description": {
                            "type": "string",
                            "description": "A detailed description of the QC task to be performed."
                        }
                    },
    ...
    async def run(self, step_index: int, task_description: str) -> dict:
        ...
        if not self.mcp_clients:
            return {"status": "error", "details": "QCAgent has no MCP clients configured."}
        ...
        if not response_message.tool_calls:
            ...
            for tool_call in response_message.tool_calls:
                ...
                try:
                    ...
                    tool_results.append({"status": "error", "details": error_detail})
